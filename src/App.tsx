...
function PortalProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    Promise.all([api.settings(), api.courses()]).then(([config, list]) => {
      setSettings(config); setCourses(list);
      if (student) setCourse(list.find((item) => item.id === student.course_id));
    }).catch(console.error);
  }, []);
...
function Countdown({ compact = false }: { compact?: boolean }) {
  ...
  const fallback = `${new Date().getFullYear()}-08-30T09:00:00+01:00`;
  ...
  return <Layout>
    <section className="hero"><div className="container hero-grid">
      <div><span className="eyebrow"><Sparkles />100% free for UNILAG aspirants</span>
        ...
        <div className="trust"><span><Check />No payment</span><span><Check />Instant results</span><span><Check />30 questions</span></div>
      </div>
      <div className="hero-panel"><div className="panel-top"><span><i /> Exam countdown</span><b>August 30</b></div><Countdown />
        <div className="mock-score"><div className="score-ring"><strong>24</strong><span>/30</span></div><div><small>Your latest score</small><h3>Excellent work!</h3><p>You are in the top 8% of aspirants.</p></div></div>
    ...
    <section className="cta-section"><div className="container cta-card"><div><span className="eyebrow light">Your next score can be better</span><h2>Ready to test your preparation?</h2><p>Join other aspirants practising before August 30.</p></div><Link className="button light" to="/register">Take a free test <ChevronRight /></Link></div></section>
  </Layout>;
}
...
function Register() {
  ...
  const [form, setForm] = useState({ full_name: "", utme_score: "", course_id: "", email: "", phone_number: "", whatsapp_number: "", jamb_registration_number: "" });
  ...
  return <Layout><section className="page"><div className="container form-layout">
    ...
    <form className="card form-card" onSubmit={submit}>
      ...
      <div className="field-row"><Field label="Phone number"><input value={form.phone_number} onChange={(e) => update("phone_number", e.target.value)} placeholder="+234..." /></Field><Field label="WhatsApp number"><input value={form.whatsapp_number} onChange={(e) => update("whatsapp_number", e.target.value)} placeholder="+234..." /></Field></div>
      <Field label="JAMB registration number"><input value={form.jamb_registration_number} onChange={(e) => update("jamb_registration_number", e.target.value)} placeholder="Optional" /></Field>
...
function Setup() {
  ...
  return <Layout><section className="page"><div className="container narrow">
    ...
    <div className="setup-grid"><div className="card"><div className="profile-line"><div className="avatar">{student.full_name[0]}</div><div><strong>{student.full_name}</strong><span>{course.name} · UTME {student.utme_score}</span></div></div><h3>Your test subjects</h3>
      ...
      <div className="card"><h3>Test details</h3><div className="stats-row"><div><strong>{settings?.total_questions || 30}</strong><span>Questions</span></div><div><strong>{settings?.max_score || 30}</strong><span>Total marks</span></div><div><strong>1</strong><span>Mark each</span></div></div>
...
function Instructions() {
  ...
  return <Layout><section className="page"><div className="container instruction-wrap"><div className="instruction-heading"><span className="icon-box large"><BookOpen /></span><span className="step-label">Step 3 of 3</span><h1>Test instructions</h1><p>Read carefully. Your timer starts immediately.</p></div>
    ...
    <div className="ready-card"><div><strong>Ready, {student.full_name.split(" ")[0]}?</strong><span>{course.name} · 30 questions · {duration} minutes</span></div><button className="button" onClick={start} disabled={loading}>{loading ? "Preparing..." : "Start test now"}<ArrowRight /></button></div>{error && <p className="error">{error}</p>}
  </div></section></Layout>;
}
...
function Test() {
  ...
  const question = session.questions[index]; const answered = Object.keys(answers).length;
  return <Layout minimal><div className="test-shell"><header className="test-header"><Brand /><div className={`test-timer ${seconds < 60 ? "danger" : ""}`}><Timer /><span>Time left</span><strong>{String(Math.floor(seconds / 60)).padStart(2,"0")}:{String(seconds % 60).padStart(2,"0")}</strong></div><button className="button outline" onClick={() => setConfirm(true)}>Submit test</button></header><div className="test-progress"><span style={{ width: `${((index + 1) / 30) * 100}%` }} /></div>
    <main className="test-main"><section className="question-area"><div className="question-meta"><span>{question.subject}</span><strong>Question {index + 1} of 30</strong></div><h2>{question.question}</h2>
      ...
      <div className="question-actions"><button className="button muted" disabled={index === 0} onClick={() => setIndex(index - 1)}><ArrowLeft />Previous</button><button className="button" disabled={index === 29} onClick={() => setIndex(index + 1)}>Next<ArrowRight /></button></div>
    </section><aside className="palette"><div className="palette-head"><h3>Question palette</h3><span>{answered}/30 answered</span></div><div className="palette-grid">{session.questions.map((item, i) => <button className={`${answers[item.id] ? "answered" : ""} ${i === index ? "current" : ""}`} onClick={() => setIndex(i)} key={item.id}>{i + 1}</button>)}</div><div className="legend"><span><i className="answered" />Answered</span><span><i />Not answered</span></div></aside></main>
  </div>{confirm && <div className="modal-backdrop"><div className="modal"><span className="icon-box warning"><ShieldCheck /></span><h2>Submit your test?</h2><p>You answered <strong>{answered} out of 30</strong>. Unanswered questions will be marked wrong.</p><div className="modal-actions"><button className="button muted" onClick={() => setConfirm(false)}>Keep answering</button><button className="button" onClick={submit} disabled={submitting}>{submitting ? "Submitting..." : "Submit test"}</button></div></div></div>}</Layout>;
}
...
function ResultPage() {
  ...
  return <Layout><section className="result-hero"><div className="container"><span className="icon-box success"><Award /></span><p>Test completed</p><h1>Well done, {result.student.full_name.split(" ")[0]}!</h1><span>{result.recommendation}</span></div></section>
    <section className="result-body"><div className="container result-grid"><div className="score-card"><div className="score-circle"><strong>{result.score}</strong><span>/ {result.max_score}</span></div><h2>{result.percentage}%</h2><p>Final score</p><div className="result-mini"><div><strong>{result.correct_answers}</strong><span>Correct</span></div><div><strong>{result.wrong_answers}</strong><span>Wrong</span></div><div><strong>{result.unanswered_questions}</strong><span>Unanswered</span></div></div></div>
      ...
      <div className="whatsapp-card"><MessageCircle /><div><h2>Improve before August 30</h2><p>Get daily questions and explanations with other aspirants.</p></div><WhatsApp label="Join free study group" /></div>
    </div></section>
  </Layout>;
}
...
function Admin() {
  ...
  const cards = [[Users,"Registered students",stats?.total_registered_students || 0],[BookOpen,"Test attempts",stats?.total_test_attempts || 0],[Target,"Average score",`${stats?.average_score || 0}/30`],[Trophy,"Highest score",`${stats?.highest_score || 0}/30`]] as const;
  return <Layout minimal><div className="admin-shell"><aside className="admin-sidebar"><Brand /><nav><a className="active"><LayoutDashboard />Overview</a><a><BookOpen />Questions</a><a><GraduationCap />Courses</a><a><Users />Students</a><a><Trophy />Leaderboard</a><a><SettingsIcon />Settings</a></nav><button onClick={() => { sessionStorage.removeItem("admin-token"); setToken(""); }}>Sign out</button></aside>
    <main className="admin-main"><div className="admin-top"><div><p>Admin workspace</p><h1>Dashboard overview</h1></div><Link className="button outline" to="/">View portal</Link></div><div className="admin-stat-grid">{cards.map(([Icon,label,value]) => <article key={label}><span className="icon-box"><Icon /></span><div><p>{label}</p><h2>{String(value)}</h2></div></article>)}</div>
      <div className="admin-grid"><div className="card"><h2>Quick actions</h2><div className="quick-actions"><button><BookOpen />Add a question<ChevronRight /></button><button><GraduationCap />Manage courses<ChevronRight /></button><button><SettingsIcon />Update exam settings<ChevronRight /></button></div></div><div className="card"><h2>Platform status</h2>{["API and database","30-question grading","Leaderboard"].map((item) => <div className="status-line" key={item}><span><i />{item}</span><strong>Operational</strong></div>)}</div></div>
    </main>
  </div></Layout>;
}
